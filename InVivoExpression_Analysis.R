...
# # # # # Read Data # # # # # #
# # # # # # # # # # # # # # # #
root="~/GitHub/Xiaoetal_2017_analysis/" #path to cuffdiff files. 
...
save(input,file = "~/GitHub/GeneExpression/allGene_cuffdiff_3-27.Rdata")

#Save time and reload dataset as Rdata
load("~/GitHub/GeneExpression/allGene_cuffdiff_3-27.Rdata")
...
siggenesKD<-as.character(unique(sigKD$gene))
siggenesLV<-as.character(unique(sigLV$gene))

selectSig<-allgenes[toupper() %in% toupper(siggenes),]
sS<-ggplot(selectSig,aes(Annot,log2.fold_change.))
sS+geom_violin()+geom_boxplot(width=0.2,cex=0.5)+coord_cartesian(ylim = c(-5,5))+facet_grid(time~genotype)+theme()
